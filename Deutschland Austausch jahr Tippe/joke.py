...
class Joke(webapp.RequestHandler):
  def get(self):
...
''')
    ...
    self.response.out.write(''' <font size="5"><p>Sorry, you are not registered</p>''')
    self.response.out.write('''<font size="3"><a href="/first" style="text-decoration:none"> go to the first page
    </a></font>''')
    #self.response.out.write('''<h1> hello </h1>''')
      
    
application = webapp.WSGIApplication([('/joke', Joke)], debug=True)
